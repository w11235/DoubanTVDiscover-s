...
class DoubanTVDiscover(_PluginBase):
    plugin_name = "新剧放送"
    plugin_desc = "探索中直接显示豆瓣电视剧，固定首播时间排序，地区为华语和韩国，过滤短片和短剧。"
        ...
        return [
            {
                ...
                "methods": ["GET"],
                "summary": "豆瓣剧集探索数据源",
                "description": "固定返回豆瓣电视剧，首播时间排序，地区为华语和韩国，过滤短片和短剧",
            }
        ]
    ...
    @staticmethod
    def __is_shortform(media: Dict[str, Any]) -> bool:
        overview = str(media.get("overview") or "").strip()
        return "短片" in overview or "短剧" in overview
        ...
        for area, result in zip(DOUBAN_AREAS, fetched):
            ...
            for media in enriched_result:
                if isinstance(media, Exception):
                    ...
                    continue
                runtime = self.__runtime_minutes(media)
                if runtime > 0 and runtime <= MIN_RUNTIME:
                    continue
                if self.__is_shortform(media):
                    continue
