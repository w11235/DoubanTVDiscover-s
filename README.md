# DoubanTVDiscover (MoviePilot v2)

独立版 `新剧放送` 插件。

当前版本使用固定筛选参数形式向探索页提供数据源，兼容性更好。
同时使用独立 `mediaid_prefix`，避免与内置豆瓣源潜在冲突，并在点击搜索时自动转换回豆瓣/TMDB ID。
当豆瓣探索列表未返回时长时，插件会自动补拉豆瓣详情；缺少时长的条目会保留，只有明确时长不超过 25 分钟的条目才会被过滤。
简介中含有 `短片` 或 `短剧` 的条目会被过滤。
探索页不显示排序/地区/时长筛选条，始终按固定条件运行；单次返回数量提升到至少 100 条。

## 目录结构

```text
DoubanTVDiscover-standalone/
├── package.v2.json
└── plugins.v2/
    └── doubantvdiscover/
        └── __init__.py
```

## 安装方式

### 方式一：作为独立插件仓库安装

1. 将本目录内容推送到你自己的 GitHub 仓库根目录。
2. 在 MoviePilot v2 中添加插件仓库地址（`package.v2.json` 的 Raw URL）。
3. 在插件市场搜索并安装 `新剧放送`。

### 方式二：手动安装

1. 将 `plugins.v2/doubantvdiscover` 目录复制到你的插件仓库 `plugins.v2/` 下。
2. 将 `package.v2.json` 中的 `DoubanTVDiscover` 条目合并到你的仓库 `package.v2.json`。
3. 重启 MoviePilot 或刷新插件市场。

## 固定筛选条件

- 类型：电视剧
- 排序：首播时间
- 地区：华语、韩国
- 时长：若能识别则要求大于 25 分钟；缺少时长则保留
- 简介：过滤含 `短片`、`短剧` 的条目

## 说明

- 插件会分别请求豆瓣剧集探索中的 `华语` 和 `韩国` 数据，再合并去重。
- 仅过滤明确识别出 `runtime <= 25` 的条目，未识别到时长的剧集会保留。
- 简介中包含 `短片` 或 `短剧` 的条目会被过滤。
- 合并后会再次按首播日期倒序排列，保证结果更接近“首播时间”排序预期。
